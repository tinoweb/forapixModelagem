...
$allowedActions = ['migrate', 'migrate:status', 'migrate:rollback'];

if (!in_array($action, $allowedActions, true)) {
    http_response_code(400);
    echo json_encode([
        'success' => false,
        'message' => 'Ação inválida. Use: ' . implode(', ', array_map(fn($a) => "?action={$a}", $allowedActions)),
    ], JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT);
    exit;
}
...
try {
    ...
    // migrate:rollback exige confirmação explícita via ?confirm=1
    if ($action === 'migrate:rollback') {
        if (($_GET['confirm'] ?? '') !== '1') {
            http_response_code(400);
            echo json_encode([
                'success' => false,
                'message' => 'Para rollback adicione &confirm=1 na URL. Atenção: esta ação é destrutiva.',
            ], JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT);
            exit;
        }
        $artisanArgs = ['--force' => true];
    }
    ...
    $exitCode = Illuminate\Support\Facades\Artisan::call($artisanCmd, $artisanArgs);
